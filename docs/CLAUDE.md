...
- **PySide6, nie PyQt6** (PyQt6 jest GPL; przy MIT używamy PySide6/LGPL). **GUI stoi na `chodzkos-gui-kit`** — NIE pisz własnego `theme.py` ani dialogów. Używaj `ThemeManager` (`apply`/`attach_titlebar`), `qt.dialogs.*`, widgetów `PathEntry`/`FileList`/`LogView`/`HelpWindow` (+ helpery `help_html`), `get_icon`/`ICON_MAP`. `GUI_STANDARD.md` mieszka w repo kitu (nie kopiuj go tutaj). Pin po **SHA** commitu z komentarzem wersji (`git+...@<SHA>  # vX.Y.Z`), **nigdy sam tag i nigdy `main`** — tagi gitowe są mutowalne, SHA nie (decyzja supply-chain z audytu 07.2026, spójna z `gui-kit/CLAUDE.md`); podniesienie wersji = osobny commit `chore:` + testy. Szczegóły: `ARCHITECTURE.md` → Integracja z chodzkos-gui-kit.
